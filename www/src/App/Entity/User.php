...
class User extends Layer
{
    ...
    {
        $query = $this->createQueryBuilder()->select("user_id")
            ->from($this->table)
            ->orderBy('user_id')
            ->executeQuery();
    ...
    {
        $query = $this->createQueryBuilder()->select("user_id, username, registration_date")
            ->from($this->table)
            ->setMaxResults(1)
    ...
    {
        $query = $this->createQueryBuilder()->update('user_friendships')
            ->set('awaiting', '?')
            ->where('sender_id = ?')
            ->andWhere('user_id = ?')
            ->setParameter(0, 0)
    ...
    {
        $query = $this->createQueryBuilder()->delete('user_friendships')
            ->where('sender_id = ?')
            ->andWhere('user_id = ?')
            ->setParameter(0, $sender_id)
            ->setParameter(1, $user_id)
    ...
    {
        $query = $this->createQueryBuilder()->update("user_bans")
            ->set('expires', '?')
            ->where('ban_id = ?')
            ->setParameter(0, 0)
