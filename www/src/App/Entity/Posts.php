        ...
        if (!is_null($forum_id))
        {
            ...
            )
            {
                ...
                $query->leftJoin(
                    ...
                    \sprintf(
                        '(%s)',
                        $this->createQueryBuilder()->select('p.dateline', 'p.user_id', 'p.post_id', 'u.username as fp_username', 'u.user_id as fp_user_id')
                            ->from('posts', 'p')
                            ->leftJoin(
                                'p',
                                \sprintf(
                                    '(%s)',
                                    $this->createQueryBuilder()->select('u.user_id', 'u.username')
                                        ->from('users', 'u')
                                ),
                                'u',
                                'u.user_id = p.user_id'
                ...
                $query->leftJoin(
                    ...
                    \sprintf(
                        '(%s)',
                        $this->createQueryBuilder()->select('discussion_id', 'count(post_id) as comment_count', 'max(post_id) as max_post_id', 'is_active')
                            ->from('posts')
                ...
                $query->leftJoin(
                    ...
                    \sprintf(
                        '(%s)',
                        $this->createQueryBuilder()->select('post_id', 'count(post_id) as like_count')
                            ->from('post_likes')
                            ->groupBy('post_id')
                    ),
                    'lp',
                ...
                $query->leftJoin(
                    ...
                    \sprintf(
                        '(%s)',
                        $this->createQueryBuilder()->select('discussion_id', 'count(view_id) as view_count')
