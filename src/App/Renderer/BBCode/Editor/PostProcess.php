...
class PostProcess
{
    ...
    private function setOption(array $key, $option)
    {
        foreach ($key as $index)
        {
            if (!is_int($index))
            {
                throw new \Exception("Key must be integer value!");
            }

            $this->modifications[$index]['option'] = $option;
        }
    }
    ...
    {
        $i = 0;
        foreach ($this->getPatterns() as $key => $val)
        {
            $i++;
